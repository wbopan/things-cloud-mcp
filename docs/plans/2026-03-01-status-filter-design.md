...
The `things_find_tasks` tool uses `is_completed` (bool) to toggle visibility of non-active tasks. This has two issues:
...
Replace `is_completed` (bool) with `status` (string enum) on `things_find_tasks` and `things_find_projects`.
...
| Tool | Change |
|---|---|
| `things_find_tasks` | Replace `is_completed` with `status` enum |
| `things_find_projects` | Add `status` enum (currently hardcoded to exclude non-pending) |
